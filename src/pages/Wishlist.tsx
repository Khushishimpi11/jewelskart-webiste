...
const Wishlist = () => {
  ...
  const handleAddToCart = (item: typeof items[0]) => {
    ...
    addToCart(cartProduct, item.selectedSize);
    removeItem(item.id);
    
    if (item.selectedSize && item.selectedSize !== 'Free Size') {
      toast.success(`${item.name} (Size ${item.selectedSize}) moved to cart`);
    } else {
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex gap-2 mt-4">
                        <button 
                          ...
                          className={`flex-1 text-xs py-2 flex items-center justify-center gap-1 rounded transition-all ${
                            product.isRingProduct && !product.selectedSize
